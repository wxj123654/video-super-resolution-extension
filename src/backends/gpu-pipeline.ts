...
export class GpuPipeline {
  ...
  composite(video: HTMLVideoElement): void {
    ...
    if (mode === "replace") {
      // For replace mode, just present the model output texture directly
      this.presentModelOutput();
      return;
    }

    // For luma_inject and overlay modes, use the composite pipeline
  ...
  blendTiles(
    ...
    // We need a uniform buffer for blend params
    const blendParamsBuffer = this.device.createBuffer({
      size: 32,
      usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST,
    });
    this.device.queue.writeBuffer(blendParamsBuffer, 0, data);
