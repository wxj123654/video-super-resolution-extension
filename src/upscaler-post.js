(function attachVideoGpuSuperResolutionPostRenderer() {
  const ns = window.VideoGpuSuperResolutionInternal;
  const { vertexShader, ecbsrCompositeShader, createProgram } = ns;

  class EcbsrCompositeRenderer {
    constructor(canvas) {
      this.canvas = canvas;
      ...
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, this.floatLinear ? gl.LINEAR : gl.NEAREST);
      gl.pixelStorei(gl.UNPACK_ALIGNMENT, 1);
    }

    uploadBaseFrame(data, width, height) {
      const gl = this.gl;
    ...
    uploadBaseFrameFromVideo(video) {
      const gl = this.gl;
  ...
  ns.EcbsrCompositeRenderer = EcbsrCompositeRenderer;
})();
